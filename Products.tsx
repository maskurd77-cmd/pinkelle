...
import { TrendingUp, TrendingDown, DollarSign, ShoppingBag, Receipt, ArrowUpRight, ArrowDownRight, Activity, Save } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const handleUpdateExchangeRate = async () => {
    setIsSavingEx(true);
    try {
      await setDoc(doc(db, 'system', 'settings'), { exchangeRate }, { merge: true });
  ...
  const { monthlyStats, chartData } = useMemo(() => {
    ...
    const salesProgress = lastMonthSales > 0 ? ((thisMonthSales - lastMonthSales) / lastMonthSales) * 100 : 0;
    ...
    if (active && payload && payload.length) {
      return (
         <div className="bg-white/90 backdrop-blur-sm p-3 border border-slate-100 rounded-xl shadow-xl text-right">
            ...
            <p className="font-bold text-pink-600 font-mono text-lg">{formatCurrency(payload[0].value)}</p>
         </div>
      );
    }
    return null;
    ...
    <div className="space-y-6 flex-1 overflow-auto custom-scrollbar lg:pr-2">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 p-5 rounded-3xl shadow-md flex flex-col sm:flex-row items-center justify-between gap-4 text-white">
         <div>
            ...
            <p className="text-xs text-slate-400 mt-1">نرخی ١ دۆلار بە دینار دیاری بکە بۆ هەژمارکردنی دروست.</p>
         </div>
         <div className="flex items-center gap-2">
            <div className="bg-slate-950/50 p-1 flex items-center rounded-xl border border-white/10">
               <span className="text-slate-400 text-sm font-bold font-mono px-3 pl-1">$1 =</span>
               <input 
                  type="number" 
                  value={exchangeRate}
               ...
               />
               <span className="text-slate-400 text-sm font-bold px-3 pr-1">د.ع</span>
            </div>
            <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-pink-500 to-rose-600 p-5 rounded-3xl shadow-lg text-white relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start relative z-10">
             <div>
               <h3 className="text-pink-100 text-sm font-medium mb-1">فرۆشتنی ئەمڕۆ</h3>
               <p className="text-3xl font-bold font-mono tracking-tight">{formatCurrency(todayStats.sales)}</p>
             </div>
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
          <div className="flex justify-between items-start">
             <div>
               <h3 className="text-slate-500 text-sm font-medium mb-1">قازانجی ئەمڕۆ</h3>
               <p className="text-2xl font-bold font-mono text-green-600">{formatCurrency(todayStats.profit)}</p>
             </div>
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex justify-between items-start">
             <div>
               <h3 className="text-slate-500 text-sm font-medium mb-1">وەسڵەکانی ئەمڕۆ</h3>
               <p className="text-2xl font-bold font-mono text-slate-800">{todayStats.count}</p>
             </div>
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex justify-between items-start">
             <div>
               <h3 className="text-slate-500 text-sm font-medium mb-1">کالاکانی فرۆشراو</h3>
               <p className="text-2xl font-bold font-mono text-slate-800">{todayStats.itemsSold}</p>
             </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
         ...
         <div className="lg:col-span-1 space-y-4">
            <h2 className="text-lg font-bold text-slate-800 mb-4 px-1">سەرژمێری مانگانە</h2>
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
               <div className="flex justify-between items-center mb-4 text-slate-600">
                  ...
                  {monthlyStats.salesProgress !== 0 && (
                     <div className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-lg ${monthlyStats.salesProgress > 0 ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                        {monthlyStats.salesProgress > 0 ? <ArrowUpRight size={14} /> : <ArrowDownRight size={14} />}
                        <span dir="ltr">{Math.abs(monthlyStats.salesProgress).toFixed(1)}%</span>
                     </div>
                  )}
               </div>
               <p className="text-3xl font-bold font-mono text-slate-800 mb-6">{formatCurrency(monthlyStats.thisMonthSales)}</p>
               
               <div className="space-y-4 mt-6 border-t border-slate-50 pt-6">
                  <div className="flex justify-between items-center">
                     <span className="text-sm text-slate-500 font-medium">مانگی پێشوو</span>
                     <span className="font-mono font-bold text-slate-700">{formatCurrency(monthlyStats.lastMonthSales)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                     <span className="text-sm text-slate-500 font-medium">باشترین مانگ <span className="text-xs bg-slate-100 px-1 ml-1 rounded text-slate-400 py-0.5" dir="ltr">{monthlyStats.bestMonth}</span></span>
                     <span className="font-mono font-bold text-green-600">{formatCurrency(monthlyStats.bestMonthVal)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                     <span className="text-sm text-slate-500 font-medium">خراپترین مانگ <span className="text-xs bg-slate-100 px-1 ml-1 rounded text-slate-400 py-0.5" dir="ltr">{monthlyStats.worstMonth}</span></span>
                     <span className="font-mono font-bold text-red-500">{formatCurrency(monthlyStats.worstMonthVal)}</span>
                  </div>
               </div>
            </div>

            <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-3xl shadow-lg text-white">
               <h3 className="text-slate-400 text-sm font-medium mb-1">فرۆشتنی ٧ ڕۆژی ڕابردوو</h3>
               <p className="text-2xl font-bold font-mono text-white">{formatCurrency(monthlyStats.last7DaysSales)}</p>
            </div>
         </div>

         {/* Chart Section */}
         <div className="lg:col-span-2">
            <h2 className="text-lg font-bold text-slate-800 mb-4 px-1">ڕێژەی فرۆشتن لە ٧ ڕۆژی ڕابردوو</h2>
