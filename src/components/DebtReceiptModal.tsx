...
export function DebtReceiptModal({
            ...
            {transaction.isGeneric ? (
              <>
                ...
                <div className="border-2 border-red-600 rounded-lg bg-red-50 p-2 text-center flex justify-between items-center px-3">
                  <span className="text-[10px] text-red-600 font-black">قەرزی ماوە (الدين Mتبقي):</span>
  ...
  return (
    <>
      {/* On-screen Modal View: Strictly hidden during print */}
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-2 sm:p-4 overflow-y-auto print:hidden">
        <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-3xl flex flex-col my-4">
          
          {/* Desktop Header */}
          <div className="px-6 py-4 justify-between items-center bg-slate-50 border-b border-slate-100 hidden md:flex rounded-t-[24px]">
            <h2 className="font-extrabold text-slate-800 text-lg flex items-center gap-2 font-sans">
              چاپی وەسڵی قەبز
            </h2>
            <div className="flex gap-2">
              <button
                onClick={() => handlePrint()}
                className="px-4 py-2 bg-pink-100 hover:bg-pink-200 text-pink-700 rounded-xl text-sm font-bold flex items-center gap-2 transition-colors shadow-sm"
              >
          ...
          <div className="md:hidden p-3 flex justify-between items-center bg-white border-b border-slate-100 pb-3">
            ...
            <button
              onClick={() => handlePrint()}
              className="px-6 py-2.5 bg-pink-600 active:bg-pink-700 text-white rounded-full text-sm font-bold flex items-center gap-2 shadow-lg"
            >
          ...
          <div className="p-4 sm:p-6 bg-slate-100 flex justify-center overflow-x-auto max-h-[70vh] overflow-y-auto">
            <div className="w-[210mm] h-[148mm] bg-white p-[6mm] box-border shadow-md rounded-xl border border-slate-200 shrink-0 relative">
      ...
      {/* Actual Landscape Print area: Only active during print */}
      <div className="hidden print:block w-[210mm] h-[148mm] text-black bg-white">
        {renderReceiptContent()}
      </div>
    </>
  );
