...
import { LineChart as LineChartIcon, PieChart as PieChartIcon, TrendingUp, Calendar, Filter } from 'lucide-react';
             ...
             {chartData.length > 0 ? (
                  ...
                  <AreaChart data={chartData} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
                     <defs>
                        ...
                        <filter id="shadow" height="200%">
                           <feDropShadow dx="0" dy="8" stdDeviation="8" floodColor="#ec4899" floodOpacity="0.25" />
                        </filter>
                     </defs>
                     ...
                     <XAxis 
                        ...
                        axisLine={false} 
                        tickLine={false} 
                        tick={{ fill: '#64748b', fontSize: 13, fontWeight: 600, fontFamily: 'monospace' }} 
                        dy={15}
                     />
                     <YAxis 
                        axisLine={false} 
                        tickLine={false} 
                        tick={{ fill: '#64748b', fontSize: 13, fontFamily: 'monospace', fontWeight: 600 }} 
                     ...
                     <Area 
                        ...
                        activeDot={{ r: 8, fill: '#fff', stroke: '#ec4899', strokeWidth: 4 }}
                ...
                <div className="h-full flex items-center justify-center text-slate-400 flex-col gap-4">
                   <div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mb-2">
                       <Filter size={48} className="text-slate-300" strokeWidth={1.5} />
                   </div>
                   <p className="text-lg font-bold text-slate-500">هیچ داتایەک بوونی نییە بۆ ئەم کاتە</p>
                </div>
             )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm p-8 flex flex-col min-h-[450px]">
          ...
          <div className="flex-1 w-full min-h-[350px]">
             {categoryData.length > 0 ? (
                   ...
                   <PieChart>
                     ...
                     >
                       {categoryData.map((entry, index) => (
                         <Cell key={`cell-${index}`} fill={`url(#pieGrad${index})`} className="drop-shadow-sm hover:opacity-90 transition-opacity outline-none" />
                       ))}
                     </Pie>
                     <Tooltip 
                     ...
                     <Legend 
                        ...
                        height={40} 
                        iconType="circle" 
                        formatter={(value) => <span className="text-sm font-medium text-slate-700 mr-1">{value}</span>}
                     />
                   </PieChart>
                </ResponsiveContainer>
             ) : (
                <div className="h-full flex items-center justify-center text-slate-400 flex-col gap-4">
                   <div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mb-2">
                       <PieChartIcon size={48} className="text-slate-300" strokeWidth={1.5} />
                   </div>
                   <p className="text-lg font-bold text-slate-500">هیچ داتایەک بوونی نییە</p>
                </div>
             )}
          </div>
        </div>
