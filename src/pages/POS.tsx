...
export default function POS({ preselectedCustomer, hideLayout }: { preselectedCustomer?: any, hideLayout?: boolean }) {
    ...
    try {
      ...
      if (customerDetails.shopName) {
        ...
        if (existingCus) {
          ...
          if (!isEditing || true) { // We update even in editing to ensure recent purchase tracking
            batch.update(doc(db, "customers", existingCus.id), {
              ...
              lat: customerDetails.lat !== undefined ? customerDetails.lat : (existingCus.lat || null),
              lng: customerDetails.lng !== undefined ? customerDetails.lng : (existingCus.lng || null),
  ...
  return (
    <div className={`flex flex-col lg:flex-row h-full gap-4 lg:gap-6 relative overflow-hidden ${hideLayout ? 'pb-20 lg:pb-0' : ''}`}>
      ...
      <div className={`flex-1 flex flex-col bg-white lg:rounded-[24px] ${hideLayout ? '' : 'lg:border border-slate-100 shadow-sm'} overflow-hidden h-full -mx-3 sm:mx-0`}>
        <div className={`p-3 lg:p-5 border-b border-pink-100/30 bg-white flex flex-col gap-3 shrink-0 z-10 sticky top-0 ${hideLayout ? 'shadow-none pt-2 lg:pt-3' : 'shadow-sm'}`}>
            ...
            <div className="flex items-center gap-2 w-full">
              ...
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-white border border-pink-200/50 rounded-xl py-2 pr-9 pl-3 text-xs focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 transition-all font-bold text-slate-800 h-9 shadow-sm shadow-pink-500/5"
                />
              </div>

              {/* Mobile Wholesale Toggle - Sleek and compact */}
              <div className="lg:hidden flex items-center bg-pink-50/50 p-0.5 rounded-xl border border-pink-100/50 h-9 shrink-0">
                <button
                  type="button"
                  onClick={() => setIsWholesale(false)}
                  className={`px-3 py-1 text-xs font-black rounded-lg transition-all duration-200 h-full flex items-center justify-center ${!isWholesale ? "bg-white shadow-sm text-pink-700" : "text-slate-500"}`}
                >
                  تاک
                </button>
                <button
                  ...
                  className={`px-3 py-1 text-xs font-black rounded-lg transition-all duration-200 h-full flex items-center justify-center ${isWholesale ? "bg-pink-600 text-white shadow-sm" : "text-slate-500"}`}
                >
                  کۆ
                </button>
              </div>
          ...
          <div className="flex gap-1.5 overflow-x-auto custom-scrollbar pb-0.5 sm:pb-0 hide-scrollbar -mx-3 px-3 sm:mx-0 sm:px-0">
            <button
              onClick={() => setSelectedCategory("all")}
              className={`px-3 py-1.5 shrink-0 rounded-lg text-[11px] sm:text-xs font-extrabold transition-all duration-200 ${selectedCategory === "all" ? "bg-pink-600 text-white shadow-md shadow-pink-500/10" : "bg-white border border-pink-100/40 text-slate-500 hover:bg-pink-50 hover:text-pink-600 active:scale-95"}`}
            >
              هەمووی
            </button>
            {categories.map((c) => (
              <button
                ...
                className={`px-3 py-1.5 shrink-0 rounded-lg text-[11px] sm:text-xs font-extrabold transition-all duration-200 ${selectedCategory === c ? "bg-pink-600 text-white shadow-md shadow-pink-500/10" : "bg-white border border-pink-100/40 text-slate-500 hover:bg-pink-50 hover:text-pink-600 active:scale-95"}`}
              >
                {c}
              </button>
            ))}
        ...
        <div className="flex-1 overflow-y-auto p-3 lg:p-5 custom-scrollbar bg-white pb-36 lg:pb-5">
      ...
      {cartItemCount > 0 && !checkoutModalOpen && (
        <div className={`mobile-floating-cart lg:hidden fixed ${hideLayout ? 'bottom-[calc(max(env(safe-area-inset-bottom),0.5rem)+1rem)]' : 'bottom-[calc(max(env(safe-area-inset-bottom),0.5rem)+5.5rem)]'} left-4 right-4 z-[100] print:hidden transition-all`}>
          <button
      ...
      {/* RIGHT AREA: Cart Sidebar (Desktop Panel & Mobile Bottom Drawer) */}
      <div
        className={`fixed bottom-0 left-0 right-0 z-[110] bg-white rounded-t-[32px] sm:rounded-t-[40px] shadow-2xl border-t border-slate-100 flex flex-col max-h-[88vh] transition-transform duration-300 ease-out transform ${
          mobileCartOpen ? "translate-y-0" : "translate-y-full"
        } lg:relative lg:translate-y-0 lg:max-h-none lg:h-full lg:w-95 lg:shadow-xs lg:rounded-[24px] lg:border lg:border-pink-50 lg:flex-none flex flex-col overflow-hidden`}
      >
        {/* Drag handle pill for mobile */}
        <div className="lg:hidden w-12 h-1.5 bg-slate-250 rounded-full mx-auto my-3 shrink-0" />

        <div className="p-4 border-b border-pink-100/30 bg-white flex flex-col gap-3 shrink-0 shadow-xs z-10 w-full pt-1 lg:pt-4">
          <div className="flex items-center justify-between w-full text-slate-900 font-bold text-lg">
            <div className="flex items-center gap-2 text-pink-600 bg-pink-50 px-3.5 py-1.5 rounded-xl font-black">
              <ShoppingCart size={18} />
              <span className="text-sm">کاشێر و سەبەتە</span>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setMobileCartOpen(false)}
                className="w-9 h-9 bg-pink-50 hover:bg-pink-100 text-pink-600 rounded-xl flex transition-all active:scale-95 lg:hidden justify-center items-center"
              >
          ...
          <div className="flex items-center p-1 bg-pink-50/50 rounded-xl border border-pink-100/40 w-full shadow-xs">
            <button
              onClick={() => setIsWholesale(false)}
              className={`py-2 text-[11px] font-black rounded-lg transition-all duration-200 flex-1 text-center ${!isWholesale ? "bg-white shadow-xs text-pink-700 scale-100" : "text-slate-500 hover:text-slate-700 scale-95"}`}
            >
              کڕیاری تاک
            </button>
            <button
              onClick={() => setIsWholesale(true)}
              className={`py-2 text-[11px] font-black rounded-lg transition-all duration-200 flex-1 text-center ${isWholesale ? "bg-pink-600 text-white shadow-xs scale-100" : "text-slate-500 hover:text-slate-700 scale-95"}`}
            >
              کڕیاری جوملە
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-white">
          {cart.length === 0 ? (
            <div className="h-full min-h-[220px] flex flex-col items-center justify-center text-slate-400 space-y-4">
            ...
            <div className="space-y-3 pb-8 lg:pb-0">
              {cart.map((item) => (
                <div
                  key={item.id}
                  className="p-3 bg-white rounded-[16px] flex gap-3.5 group relative border border-slate-200/80 shadow-xs hover:border-pink-200 transition-all text-slate-800"
                >
                  <div className="w-12 h-12 bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl flex items-center justify-center text-slate-300 shrink-0 border border-slate-100/50 overflow-hidden relative">
                    {item.imageUrl ? (
                      <img
                        src={item.imageUrl}
                        alt={item.name}
                  ...
                  <div className="flex-1 min-w-0 py-0.5 relative">
                    <h4 className="text-[13px] font-black text-slate-800 truncate mb-1.5 pl-4">
                      {item.name}
                    </h4>
                    ...
                    userRole === "admin" ? (
                      <div className="flex items-center gap-1">
                        <span
                          className={`text-[11px] font-black ${isWholesale ? "text-pink-600" : "text-slate-500"}`}
                        >
                          {isWholesale ? "جووملە:" : "تاک:"}
                        </span>
                        <input
                          type="number"
                          min="0"
                          step="any"
                          className="w-16 text-[11px] font-mono font-black border-b border-pink-400/60 focus:border-pink-550 outline-none max-w-full bg-transparent p-0 m-0"
                          value={
                      ...
                      <p
                        className={`text-[11px] font-mono font-black ${isWholesale ? "text-pink-600" : "text-pink-600"}`}
                      >
                        {isWholesale ? "جوملە: " : "تاک: "}
                        {formatCurrency(
                          item.editedPrice !== undefined
                  ...
                  <div className="flex flex-col items-end justify-between self-stretch">
                    <button
                      ...
                      className="text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors p-1 -mr-1 -mt-1"
                    >
                      <Trash2 size={15} />
                    </button>
                    <div className="flex flex-col items-end gap-1.5 mt-1.5">
                      <div className="flex items-center gap-1.5 bg-pink-50/20 rounded-xl border border-pink-100/50 p-0.5">
                        <button
                          onClick={() => updateQuantity(item.id, 1)}
                          disabled={getActualPieceQuantity({ ...item, quantity: item.quantity + 1 }) > item.stock}
                          className="w-6.5 h-6.5 shrink-0 bg-white rounded-lg shadow-xs border border-pink-100 flex items-center justify-center text-slate-600 hover:text-pink-600 outline-none disabled:opacity-50 transition-colors"
                        >
                        ...
                        <input
                          ...
                          className="w-9 text-xs font-black text-center font-mono bg-transparent outline-none p-0 m-0"
                          dir="ltr"
                        />
                        <button
                          onClick={() => updateQuantity(item.id, -1)}
                          disabled={item.quantity <= 0}
                          className="w-6.5 h-6.5 shrink-0 bg-white rounded-lg shadow-xs border border-pink-100 flex items-center justify-center text-slate-600 hover:text-red-500 outline-none disabled:opacity-50 transition-colors"
                        >
                      ...
                      {(item.cartonSize || 0) > 1 && (
                        <select
                          ...
                          className="text-[9px] font-black bg-pink-50 text-pink-700 border border-pink-100/60 rounded px-1.5 py-0.5 outline-none"
                        >
        ...
        <div className="p-4 lg:p-6 border-t border-slate-100 bg-slate-50 shrink-0 z-10 pb-[max(calc(env(safe-area-inset-bottom)+1rem),1.5rem)] lg:pb-5">
          <div className="space-y-3 mb-4.5">
            <div className="flex justify-between items-center text-[12px] text-slate-500 font-bold">
              <span>هەموو کاڵاکان ({cartItemCount})</span>
            ...
            {(userPermissions.includes("pos_allow_discount") ||
              userRole === "admin") && (
              <div className="flex justify-between items-center text-[12px] text-slate-500 font-bold border-t border-slate-200/50 pt-2.5">
                <span className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="text-[10px] bg-white border border-pink-200/60 rounded p-1 outline-none font-black text-pink-700"
                  >
                    <option value="amount">بڕی دۆلار</option>
                ...
                <input
                  ...
                  className="w-20 text-xs font-mono font-black border border-pink-205 rounded-lg py-1 px-2 focus:border-pink-500 outline-none focus:ring-1 focus:ring-pink-550 text-left bg-white transition-colors"
                  dir="ltr"
                />
              </div>
            )}

            <div className="pt-2.5 border-t text-pink-600 border-slate-200/80 flex justify-between items-end">
              <span className="text-[14px] font-black text-slate-850 tracking-tight">
                کۆی گشتی نێردراو
              </span>
              <span className="text-xl font-black font-mono tracking-tight text-pink-700">
                {formatCurrency(total)}
              </span>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 bg-pink-600 focus-visible:ring-4 focus-visible:ring-pink-500/30 hover:bg-pink-500 disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none text-white py-3 sm:py-3.5 rounded-xl font-black text-[13px] transition-all flex items-center justify-center gap-2 shadow-md shadow-pink-600/10 active:scale-[0.98]"
            >
              <CreditCard size={18} />
              تەواوکردنی فرۆشتن و واژۆکردن
            </button>
          </div>
        </div>
      </div>

      {/* Overlay backdrop for mobile cart */}
      {mobileCartOpen && (
        <div
          className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] lg:hidden transition-all duration-300 opacity-100"
          onClick={() => setMobileCartOpen(false)}
        />
      )}
      ...
      {/* CHECKOUT MODAL */}
      {checkoutModalOpen && (
        <div className="fixed inset-0 z-[120] flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm bg-slate-900/40 transition-opacity">
          <div className="bg-white rounded-t-[24px] sm:rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90dvh] sm:max-h-[85vh] animate-in slide-in-from-bottom duration-250">
            {saleCompleted ? (
              <div className="p-8 flex flex-col items-center justify-center text-center overflow-y-auto">
                <div className="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center text-emerald-600 mb-4 shrink-0 shadow-sm border border-emerald-100">
                  <CheckCircle2 size={32} />
                </div>
                <h2 className="text-xl font-extrabold text-slate-900 mb-1">
                  فرۆشتنەکە سەرکەوتوو بوو
                </h2>
                <p className="text-xs font-bold text-slate-500 mb-6 max-w-xs leading-relaxed">
                  پسوولەکە بە سەرکەوتوویی تۆمارکرا لە سیستەمەکەدا.
                </p>
                <div className="flex gap-2.5 w-full">
                  <button
                    onClick={resetPOS}
                    className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-xl transition-colors text-sm"
                  >
                    فرۆشتنی نوێ
                  </button>
                </div>
              ...
              <>
                <div className="px-5 py-4 border-b border-pink-100/30 flex items-center justify-between bg-white shrink-0">
                  <h2 className="text-base font-black text-slate-800 flex items-center gap-2">
                    <Receipt className="text-pink-600" size={18} />
                    زانیاری کڕیار / دوکان
                  </h2>
                  <button
                  ...
                  >
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-slate-500 flex items-center gap-1.5">
                        <Banknote size={14} className="text-pink-500" /> جۆری پێدان <span className="text-red-500">*</span>
                      </label>
                      <div className="grid grid-cols-2 gap-2.5">
                        <button
                          type="button"
                          onClick={() => setCustomerDetails({ ...customerDetails, paymentType: "cash" })}
                          className={`py-2.5 px-3 rounded-xl border-2 font-bold text-xs flex items-center justify-center gap-1.5 transition-all duration-200 ${
                            customerDetails.paymentType === "cash" || customerDetails.paymentType === "نەقد"
                              ? "border-pink-600 bg-pink-50/50 text-pink-700 shadow-sm"
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <select
                          ...
                          className="w-full bg-white border border-pink-200/60 rounded-xl py-3 px-4 focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 transition-all font-bold text-slate-800 appearance-none mb-2 shadow-sm shadow-pink-500/5"
                          value={
                            customers.find(
                              (c) => c.name === customerDetails.shopName,
                            )
                      ...
                      {!customers.find(
                        ...
                        <input
                          ...
                          className="w-full bg-white border border-pink-200/60 rounded-xl py-3 px-4 focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 transition-all font-bold text-slate-800 shadow-sm shadow-pink-500/5"
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full bg-white border border-pink-200/60 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-pink-500/20 font-mono text-left text-sm shadow-sm shadow-pink-500/5"
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <textarea
                        ...
                        className="w-full bg-white border border-pink-200/60 rounded-xl py-3 px-4 focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 transition-all font-bold text-slate-800 resize-none shadow-sm shadow-pink-500/5"
